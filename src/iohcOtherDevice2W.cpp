...
//                }
            ...
            //         digitalWrite(RX_LED, digitalRead(RX_LED)^1);
            //         std::vector<uint8_t> toSend = {0x0C, 0x61, 0x01, 0xFF, 0x00};// Accepted command {0x0C, 0x61, 0x11, 0x00, 0x00/01}; Answer  {0x0C, 0x61, 0x11, 0x00, 0x00}
            //         int custom = atoi(data);
            //         toSend[3] = custom;
            ...
            //         digitalWrite(RX_LED, digitalRead(RX_LED)^1);
            //         std::vector<uint8_t> toSend = {0x00, 0x0c, 0x00, 0x00, 0x03, 0x00, 0x00, 0x01, 0x53}; //{0x0C, 0x60, 0x01, 0x2C};

            //         packets2send[0]->payload.packet.header.cmd = 0x48;
            //         memorizeSend.memorizedData = toSend;
            //         memorizeSend.memorizedCmd = 0x48;
            ...
            //         packets2send[0]->stamp = 25;
            //         packets2send[0]->repeat = 1; //0; // Need to stop txMode
            //         packets2send[0]->lock = false; //true; // Need to received ASAP
            //         packets2send[0+1] = nullptr;
            ...
            //         digitalWrite(RX_LED, digitalRead(RX_LED)^1);
            //         /*std::vector<uint8_t>*/uint8_t toSend[] = {};

            //         uint8_t broadcast[3];
            //         hexStringToBytes(data, broadcast);
            //         packets2send[0]->payload.packet.header.cmd = 0x28;
            //         memorizeSend.memorizedData.assign(toSend, toSend + sizeof(toSend));
            //         memorizeSend.memorizedCmd = 0x28;

            //         packets2send[0]->payload.packet.header.CtrlByte1.asStruct.StartFrame = 1;
            //         packets2send[0]->payload.packet.header.CtrlByte1.asStruct.EndFrame = 1;
            //         packets2send[0]->payload.packet.header.CtrlByte1.asByte += sizeof(toSend); ///*.size()*/ + 8;

            //         memcpy(packets2send[0]->payload.packet.header.source, gateway, 3);
            //         memcpy(packets2send[0]->payload.packet.header.target, broadcast/*associated_to*/, 3);
            //         memcpy(packets2send[0]->payload.buffer + 9, toSend, sizeof(toSend));

            //         packets2send[0]->buffer_length = sizeof(toSend) + 9; //packet2send[0]->payload.packet.header.framelength +1;
            //         packets2send[0]->frequency = CHANNEL3;
            //         packets2send[0]->stamp = 25;
            //         packets2send[0]->repeat = 4; //0; // Need to stop txMode
            //         packets2send[0]->lock = false; //true; // Need to received ASAP
            //         packets2send[0+1] = nullptr;
            ...
            // }
            //         break;
            //     case DeviceButton::discover2A_1:
            //     case DeviceButton::discover2A_2: {
            //         digitalWrite(RX_LED, digitalRead(RX_LED)^1);
            //         /*std::vector<uint8_t>*/uint8_t toSend[] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x10, 0x11, 0x12};

            //         uint8_t broadcast[3];
            //         hexStringToBytes(data, broadcast);

            //         packets2send[0]->payload.packet.header.cmd = 0x2A;
            //         memorizeSend.memorizedData.assign(toSend, toSend + 12);
            //         memorizeSend.memorizedCmd = 0x2A;
            ...
            //         memcpy(packets2send[0]->payload.packet.header.source, gateway, 3);
            //         memcpy(packets2send[0]->payload.packet.header.target, broadcast/*associated_to*/, 3);
            //         memcpy(packets2send[0]->payload.buffer + 9, toSend, 12);

            //         packets2send[0]->buffer_length = sizeof(toSend) + 9; //packet2send[0]->payload.packet.header.framelength +1;
            //         packets2send[0]->frequency = CHANNEL3;
            //         packets2send[0]->stamp = 25;
            //         packets2send[0]->repeat = 4; //0; // Need to stop txMode
            //         packets2send[0]->lock = false; // Need to received ASAP
            //         packets2send[0+1] = nullptr;
