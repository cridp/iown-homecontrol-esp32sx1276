...
//        printf("%lu %lu +%lu\n", packetStamp, relStamp, packetStamp - relStamp); 
        ...
        printf("(%2.2u) %1xW S %s E %s ", this->payload.packet.header.CtrlByte1.asStruct.MsgLen, this->payload.packet.header.CtrlByte1.asStruct.Protocol?1:2, this->payload.packet.header.CtrlByte1.asStruct.StartFrame?"1":"0", this->payload.packet.header.CtrlByte1.asStruct.EndFrame?"1":"0");
...
//            Serial.print(commandName);
        ...
        printf("\tFROM %2.2X%2.2X%2.2X TO %2.2X%2.2X%2.2X CMD %2.2X",
            this->payload.packet.header.source[0], this->payload.packet.header.source[1], this->payload.packet.header.source[2],
            this->payload.packet.header.target[0], this->payload.packet.header.target[1], this->payload.packet.header.target[2],
...
16:10:27.594 > (24) 1W S 1 E 1  FROM 9A5CA0 TO 00003F CMD 20 <  DATA(16)  02ff 0143 0b05ff00      SEQ 1B47 MAC 5be27b33d3c2  Type All 
        */
        if (this->payload.packet.header.cmd == 0x00 || this->payload.packet.header.cmd == 0x01) {
            auto main = static_cast<unsigned>((this->payload.packet.msg.p0x00.main[0] << 8) | this->payload.packet.msg.p0x00.main[1]);
            printf(" Org %X Acei %X Main %X fp1 %X fp2 %X ", this->payload.packet.msg.p0x00.origin, this->payload.packet.msg.p0x00.acei.asByte, main, this->payload.packet.msg.p0x00.fp1, this->payload.packet.msg.p0x00.fp2);
